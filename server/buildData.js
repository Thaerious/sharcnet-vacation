...
import { countWeekdays, nextWeekday } from "./weekdays.js";
...
function expandDatesInRecord(record) {
    let endDate = new Date(record.end_date);
...
function addURLsToData(data, hash) {
    const acceptURL = new URL(process.env.SERVER_NAME + CONST.PATH.ACCEPT);
    acceptURL.searchParams.append("hash", hash);

    const rejectURL = new URL(process.env.SERVER_NAME + CONST.PATH.REJECT);
...
export { internationalizeDates, expandDatesInRecord, addURLsToData, addManagersToData, humanizeDates };